...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@spprt_Kepthon"))
...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@kepthon"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await sython.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا :   𝐊𝐄𝐏𝐓𝐇𝐎𝐍"
    )
...
async def update(event):
    await event.edit("**جاري تحديث سورس كيبثون**")
...
async def _(event):
    ...
    joinu = await sython(JoinChannelRequest('kepthon'))
    ...
    for i in range(100):
        ...
        if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
            await sython.send_message(event.chat_id, f"**تم الانتهاء من التجميع | KE**")
    ...
    await sython.send_message(event.chat_id, "**تم الانتهاء من التجميع | KE**")
